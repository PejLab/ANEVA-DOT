...
#' @param Vg A numeric value for the estimated dosage standard deviation
...
#' @param Vg a vector with (log2 transformed???) standard deviation estimates for dosage distributions
#' associated with count data. Vg vector must be in one-to-one correspondence with ASE data, and
#' ordered correctly.
...
ANEVAdot<-function(filepath, output_columns = c("refCount","altCount"), eh1 = "refCount",
  ...
  for (i in 1:nrow(dat)){
    ...
    if (dat[i,eh1]==dat[i,eh2]){
      ...
      output$p.val[i]<-integrate(integrand,-rad,rad,eh1=Lh,eh2=Hh,Vg =Vg[i], abs.tol = 0, rel.tol = 1e-4)$value
    }
    else{
      output$p.val[i]<-NA
    }
